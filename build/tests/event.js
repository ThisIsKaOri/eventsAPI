...
describe("events", () => {
    ...
    describe("reading events", async () => {
        ...
        it("filtered location events, 200", async () => {
            const { status, body } = await (0, supertest_1.default)(app_1.app).get(`${baseURL}?location=MaCatania`);
            status.should.be.equal(200);
            body.should.have.property("length").equal(2);
        });
        it("filtered name events, 200", async () => {
            const { status, body } = await (0, supertest_1.default)(app_1.app).get(`${baseURL}?name=Phase2`);
            status.should.be.equal(200);
            body.should.have.property("length").equal(1);
        });
        it("filtered date events, 200", async () => {
            const { status, body } = await (0, supertest_1.default)(app_1.app).get(`${baseURL}?date=2023-04-22`);
            status.should.be.equal(200);
            body.should.have.property("length").equal(2);
        });
    });
